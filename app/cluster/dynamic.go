...
func (d *Dynamic) stop() {
	...
	start := time.Now()
	d.Processor.Stop()
	d.logger.Info("Processor stopped")
	d.Router.Stop()
	d.logger.Info("Router stopped")
	d.MultiTenantStat.Stop()
	d.logger.Info("MultiTenantStat stopped")

	d.RouterDB.Stop()
	d.logger.Info("RouterDB stopped")
	d.BatchRouterDB.Stop()
	d.logger.Info("BatchRouterDB stopped")
	d.ErrorDB.Stop()
	d.logger.Info("ErrorDB stopped")
	d.GatewayDB.Stop()
	d.logger.Info("GatewayDB stopped")
