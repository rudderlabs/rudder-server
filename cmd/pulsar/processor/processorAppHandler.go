...
func (a *processorApp) StartRudderCore(ctx context.Context, options *app.Options) error {
	...
	// terminalErrFn := terminalErrorFunction(ctx, g) // TODO?
...
func (a *processorApp) startHealthWebHandler(ctx context.Context) error {
	// Port where Processor health handler is running
	a.log.Infof("Starting in %d", a.config.http.webPort)
	srvMux := chi.NewMux()
	// srvMux.HandleFunc("/health", app.LivenessHandler(db)) // TODO use pulsar client here
	// srvMux.HandleFunc("/", app.LivenessHandler(db)) // TODO use pulsar client here
