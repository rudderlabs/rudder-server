...
func querySnowflake(anonymousId string, table string, schema string, destConfig interface{}) QueryTrackPayload{
	...
	fmt.Println(fmt.Sprintf(`select label, category, property_1, property_2, property_3, property_4, property_5 from %s where ANONYMOUS_ID='%s'`,strings.ToUpper(table), anonymousId))
	row:=db.QueryRow(fmt.Sprintf(`select label, category, property_1, property_2, property_3, property_4, property_5 from snowflaketest.%s where ANONYMOUS_ID='%s'`,strings.ToUpper(table), anonymousId))
