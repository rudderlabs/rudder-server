...
func (gw *Handle) Setup(
	...
	config.RegisterIntConfigVariable(8089, &gw.conf.adminWebPort, false, 1, "Gateway.adminWebPort")
	...
	// Registering stats
	gw.batchSizeStat = gw.stats.NewStat("gw.batch_size", stats.HistogramType)
	gw.requestSizeStat = gw.stats.NewStat("gw.request_size", stats.HistogramType)
	gw.dbWritesStat = gw.stats.NewStat("gw.db_writes", stats.CountType)
	gw.dbWorkersBufferFullStat = gw.stats.NewStat("gw.db_workers_buffer_full", stats.CountType)
	gw.dbWorkersTimeOutStat = gw.stats.NewStat("gw.db_workers_time_out", stats.CountType)
	gw.bodyReadTimeStat = gw.stats.NewStat("gw.http_body_read_time", stats.TimerType)
	gw.addToWebRequestQWaitTime = gw.stats.NewStat("gw.web_request_queue_wait_time", stats.TimerType)
	gw.addToBatchRequestQWaitTime = gw.stats.NewStat("gw.batch_request_queue_wait_time", stats.TimerType)
	gw.processRequestTime = gw.stats.NewStat("gw.process_request_time", stats.TimerType)
	gw.emptyAnonIdHeaderStat = gw.stats.NewStat("gw.empty_anonymous_id_header", stats.CountType)
	...
	gw.rerh = &ReplayRequestHandler{Handle: gw}
	gw.webhookHandler = webhook.Setup(gw, gw.stats)
...
func (gw *Handle) initUserWebRequestWorkers() {
	...
	for i := 0; i < gw.conf.maxUserWebRequestWorkerProcess; i++ {
		...
		userWebRequestWorker := &userWebRequestWorkerT{
			...
			batchTimeStat:  gw.stats.NewStat("gw.batch_time", stats.TimerType),
			bufferFullStat: gw.stats.NewStat("gw.user_request_worker_buffer_full", stats.CountType),
			timeOutStat:    gw.stats.NewStat("gw.user_request_worker_time_out", stats.CountType),
		}
			...
			for _, source := range wsConfig.Sources {
				...
				if source.Enabled {
					...
					if source.SourceDefinition.Category == "webhook" {
						...
						gw.webhookHandler.Register(source.SourceDefinition.Name)
					}
				}
			}
		}
		gw.conf.configSubscriberLock.Lock()
		gw.conf.writeKeysSourceMap = newWriteKeysSourceMap
		gw.conf.enabledWriteKeyWebhookMap = newEnabledWriteKeyWebhookMap
		gw.conf.enabledWriteKeyWorkspaceMap = newEnabledWriteKeyWorkspaceMap
		gw.conf.configSubscriberLock.Unlock()
		closeConfigChan(len(gw.conf.writeKeysSourceMap))
	}
}
	...
	for breq := range gw.batchUserWorkerBatchRequestQ {
		...
		err := gw.jobsDB.WithStoreSafeTx(ctx, func(tx jobsdb.StoreSafeTx) error {
			if gw.conf.gwAllowPartialWriteWithErrors {
				...
				if err != nil {
					gw.logger.Errorf("Store into gw db failed with error: %v", err)
...
func (gw *Handle) StartWebHandler(ctx context.Context) error {
	...
	srvMux.Route("/internal", func(r chi.Router) {
		...
		r.Post("/v1/replay/{{sourceId}}", gw.replayHandler)
	})
	...
	srvMux.Route("/v1", func(r chi.Router) {
		...
		r.Post("/webhook", gw.webhookHandler.RequestHandler)

		r.Get("/webhook", gw.webhookHandler.RequestHandler)
...
func (gw *Handle) Shutdown() error {
	gw.backgroundCancel()
	if err := gw.webhookHandler.Shutdown(); err != nil {
		return err
	}
