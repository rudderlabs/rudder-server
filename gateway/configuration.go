...
func loadConfig() {
	...
	//are available
	config.RegisterDurationConfigVariable("Gateway.userWebRequestBatchTimeoutInMS", time.Duration(15), &userWebRequestBatchTimeout, true, time.Millisecond)
	config.RegisterDurationConfigVariable("Gateway.dbBatchWriteTimeoutInMS", time.Duration(5), &dbBatchWriteTimeout, true, time.Millisecond)
	...
	config.RegisterIntConfigVariable("Gateway.maxReqSizeInKB", 4000, &maxReqSize, true, 1024)
	// Enable rate limit on incoming events. false by default
	config.RegisterBoolConfigVariable("Gateway.enableRateLimit", false, &enableRateLimit, true)
	...
	config.RegisterBoolConfigVariable("Gateway.allowReqsWithoutUserIDAndAnonymousID", false, &allowReqsWithoutUserIDAndAnonymousID, true)
	config.RegisterBoolConfigVariable("Gateway.allowPartialWriteWithErrors", false, &gwAllowPartialWriteWithErrors, true)
}
