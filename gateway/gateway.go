	...
	for breq := range gateway.batchRequestQ {
		...
		for _, req := range breq.batchRequest {
			...
			result.ForEach(func(_, _ gjson.Result) bool {
				if !gjson.GetBytes(body, fmt.Sprintf(`batch.%v.anonymousId`, index)).Exists() || strings.TrimSpace(gjson.GetBytes(body, fmt.Sprintf(`batch.%v.anonymousId`, index)).String()) == "" {
					body, _ = sjson.SetBytes(body, fmt.Sprintf(`batch.%v.anonymousId`, index), newAnonymousID)
				}
				if !gjson.GetBytes(body, fmt.Sprintf(`batch.%v.messageId`, index)).Exists() || strings.TrimSpace(gjson.GetBytes(body, fmt.Sprintf(`batch.%v.messageId`, index)).String()) == "" {
