...
func loadConfig() {
	...
	config.RegisterIntConfigVariable("Gateway.webhook.maxBatchSize", 32, &maxWebhookBatchSize, true, 1)
	// Timeout after which batch is formed anyway with whatever webhooks are available
	config.RegisterDurationConfigVariable("Gateway.webhook.batchTimeoutInMS", time.Duration(20), &webhookBatchTimeout, true, time.Millisecond)
