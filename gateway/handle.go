...
func (gw *Handle) getJobDataFromRequest(req *webRequestT) (jobData *jobFromReq, err error) {
	...
	if !gjson.ValidBytes(body) {
		err = errors.New(strings.ToLower(response.InvalidJSON))
		return
	}
	...
	if req.reqType != "batch" && req.reqType != "replay" && req.reqType != "retl" {
		...
		if err != nil {
			err = errors.New(strings.ToLower(response.NotRudderEvent))
			return
		}
	...
	for idx, v := range eventsBatch {
		...
		if gw.isNonIdentifiable(anonIDFromReq, userIDFromReq, eventTypeFromReq) {
			err = errors.New(strings.ToLower(response.NonIdentifiableRequest))
			return
		}
	...
	if len(body) > gw.conf.maxReqSize.Load() && !containsAudienceList {
		err = errors.New(strings.ToLower(response.RequestBodyTooLarge))
		return
	}
...
func (gw *Handle) getPayloadFromRequest(r *http.Request) ([]byte, error) {
	if r.Body == nil {
		return []byte{}, errors.New(strings.ToLower(response.RequestBodyNil))
	}
	...
	if err != nil {
		...
		return payload, errors.New(strings.ToLower(response.RequestBodyReadFailed))
	}
	return payload, nil
}
...
func (gw *Handle) extractJobsFromInternalBatchPayload(reqType string, body []byte) (
	...
	if err != nil {
		...
		gw.logger.Errorn("invalid json in request",
			obskit.Error(err))
		return nil, errors.New(strings.ToLower(response.InvalidJSON))
	}
	gw.requestSizeStat.Observe(float64(len(body)))
	...
	if len(messages) == 0 {
		...
		return nil, errors.New(strings.ToLower(response.NotRudderEvent))
	}
	...
	for _, msg := range messages {
		...
		if err != nil {
			...
			gw.logger.Errorn("invalid message in request",
				loggerFields...)
			stat.RequestEventsFailed(1, strings.ToLower(response.InvalidStreamMessage))
			stat.Report(gw.stats)
			return nil, errors.New(strings.ToLower(response.InvalidStreamMessage))
		}
		...
		if msg.Properties.RequestType != "" {
			...
			default:
				...
				if err != nil {
					stat.RequestEventsFailed(1, strings.ToLower(response.NotRudderEvent))
					stat.Report(gw.stats)
					loggerFields := msg.Properties.LoggerFields()
					loggerFields = append(loggerFields, obskit.Error(err))
		...
		if changed {
			...
			if err != nil {
				stat.RequestFailed(strings.ToLower(response.NotRudderEvent))
				stat.Report(gw.stats)
