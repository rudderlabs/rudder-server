...
To understand what the project is about, you must read the [README.md](../README.md) file before reviewing any code. 
Such context should then be used not just to review the code, but design decisions when
reviewing a branch against `main`, while keeping different aspects in mind (such as the architecture, security,
observability, performance, testability, testing coverage, and code maintainability).
...
Use `NewTaggedStat` for creating tagged metrics:

```go
stats.NewTaggedStat("metric_name", stats.TimerType, stats.Tags{"key": "value"}).SendTiming(duration)
stats.NewTaggedStat("metric_name", stats.CountType, stats.Tags{"key": "value"}).Increment()
stats.NewTaggedStat("metric_name", stats.GaugeType, stats.Tags{"key": "value"}).Gauge(value)
```

Available stat types: `stats.TimerType`, `stats.CountType`, `stats.GaugeType`, `stats.HistogramType`.

Always pass `stats.Stats` via dependency injection, never use a global instance.
...
Avoid using "failure", or "error", or similar word when wrapping. Instead focus on describing the behaviour that caused the issue.
