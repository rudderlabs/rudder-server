...
func prepareEventSchemasHandle(proc *Handle, c *testContext) *Handle {
...
func TestEventSchemasV2(t *testing.T) {
	initProcessor()

	t.Run("event schemas DB", func(t *testing.T) {
		t.Run("should process events and write to event schemas DB", func(t *testing.T) {
			...
			processor := prepareEventSchemasHandle(NewHandle(config.Default, mockTransformer), c)
		...
		t.Run("should process events and write to event schemas DB with enableParallelScan", func(t *testing.T) {
			c, cleanup := setupTest(t)
			defer cleanup()
