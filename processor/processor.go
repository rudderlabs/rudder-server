							...
							if err != nil {
								...
								if proc.config.storeSamplerEnabled.Load() {
									if proc.storeSamplingFileManager == nil {
										proc.logger.Errorn("Cannot upload as sampling file manager is nil")
									} else {
										batchDestJobsJSON, err := jsonrs.Marshal(batchDestJobs)
										if err != nil {
											return fmt.Errorf("marshalling batch router jobs: %w: %w ", err, storeErr)
										}
...
func getStoreSamplingUploader(conf *config.Config, log logger.Logger) (*filemanager.S3Manager, error) {
	var (
		bucket           = conf.GetStringVar("rudder-customer-sample-payloads", "Processor.Sampling.Bucket")
		regionHint       = conf.GetStringVar("us-east-1", "Processor.Sampling.RegionHint", "AWS_S3_REGION_HINT")
		endpoint         = conf.GetStringVar("", "Processor.Sampling.Endpoint")
		accessKeyID      = conf.GetStringVar("", "Processor.Sampling.AccessKey", "AWS_ACCESS_KEY_ID")
		secretAccessKey  = conf.GetStringVar("", "Processor.Sampling.SecretAccessKey", "AWS_SECRET_ACCESS_KEY")
		s3ForcePathStyle = conf.GetBoolVar(false, "Processor.Sampling.S3ForcePathStyle")
		disableSSL       = conf.GetBoolVar(false, "Processor.Sampling.DisableSSL")
		enableSSE        = conf.GetBoolVar(false, "Processor.Sampling.EnableSSE", "AWS_ENABLE_SSE")
	)
	...
	return filemanager.NewS3Manager(s3Config, log.Withn(logger.NewStringField("component", "proc-uploader")), func() time.Duration {
		return conf.GetDuration("Processor.Sampling.Timeout", 120, time.Second)
	})
}
