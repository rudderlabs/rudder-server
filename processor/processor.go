...
func (proc *HandleT) getFailedEventJobs(response transformer.ResponseT, commonMetaData transformer.MetadataT, eventsByMessageID map[string]types.SingularEventT, stage string) []*jobsdb.JobT {
	...
	for _, failedEvent := range response.FailedEvents {
		...
		newFailedJob := jobsdb.JobT{
			...
			Parameters:   []byte(fmt.Sprintf(`{"source_id": "%s", "destination_id": "%s", "job_run_id": "%s", "error": %s, "status_code": "%v", "stage": "%s"}`, commonMetaData.SourceID, commonMetaData.DestinationID, commonMetaData.JobRunID, string(marshalledErr), failedEvent.StatusCode, stage)),
			CreatedAt:    time.Now(),
			ExpireAt:     time.Now(),
...
func (proc *HandleT) processJobsForDest(jobList []*jobsdb.JobT, parsedEventList [][]types.SingularEventT) {
	...
	for srcAndDestKey, eventList := range groupedEvents {
		...
		commonMetaData := transformer.MetadataT{
			...
			JobRunID:        eventList[0].Metadata.JobRunID,
		...
		for _, destEvent := range destTransformEventList {
			...
			newJob := jobsdb.JobT{
				UUID:         id,
				UserID:       rudderID,
				Parameters:   []byte(fmt.Sprintf(`{"source_id": "%v", "destination_id": "%v", "job_run_id": "%v", "received_at": "%v", "transform_at": "%v", "message_id" : "%v" , "gateway_job_id" : "%v"}`, sourceID, destID, jobRunID, receivedAt, transformAt, messageId, jobId)),
				CreatedAt:    time.Now(),
				ExpireAt:     time.Now(),
				CustomVal:    destType,
