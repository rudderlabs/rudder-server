...
func (proc *Handle) getDestTransformerEvents(response transformer.ResponseT, commonMetaData *transformer.MetadataT, destination *backendconfig.DestinationT, stage string, trackingPlanEnabled, userTransformationEnabled bool) ([]transformer.TransformerEventT, []*types.PUReportedMetric, map[string]int64, map[string]MetricMetadata) {
	...
	for i := range response.Events {
		...
		proc.updateMetricMaps(successCountMetadataMap, successCountMap, connectionDetailsMap, statusDetailsMap, userTransformedEvent, jobsdb.Succeeded.State, stage, func() json.RawMessage { return []byte(`{}`) })
...
func (proc *Handle) updateMetricMaps(countMetadataMap map[string]MetricMetadata, countMap map[string]int64, connectionDetailsMap map[string]*types.ConnectionDetails, statusDetailsMap map[string]map[string]*types.StatusDetail, event *transformer.TransformerResponseT, status, stage string, payload func() json.RawMessage) {
	if proc.isReportingEnabled() {
		...
		if stage == types.TRACKINGPLAN_VALIDATOR && status == types.SUCCEEDED {
			if veCount > 0 {
				...
				status = types.SUCCEEDED_WITHOUT_VIOLATION
			}
		}
		...
		if ok {
			...
			for eventIndex, singularEvent := range singularEvents {
				...
				if proc.isReportingEnabled() {
					...
					proc.updateMetricMaps(inCountMetadataMap, inCountMap, connectionDetailsMap, statusDetailsMap, event, jobsdb.Succeeded.State, types.GATEWAY, func() json.RawMessage {
