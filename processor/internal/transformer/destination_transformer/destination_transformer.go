...
func New(conf *config.Config, log logger.Logger, stat stats.Stats, opts ...Opt) *Client {
	...
	handle.config.gzipCompress = conf.GetReloadableBoolVar(false, "Processor.DestinationTransformer.gzipCompress", "Processor.Transformer.gzipCompress", "Transformer.gzipCompress")
...
func (d *Client) doPost(ctx context.Context, rawJSON []byte, url string, labels types.TransformerMetricLabels, extraHeaders map[string]string) ([]byte, int, error) {
	...
	err := backoffvoid.Retry(ctx,
		transformerutils.WithProcTransformReqTimeStat(func() error {
			...
			d.stat.NewTaggedStat("transformer_client_request_total_bytes", stats.CountType, tags).Count(reqBytesLen)
