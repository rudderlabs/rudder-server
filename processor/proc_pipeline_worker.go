...
func newProcPipelineWorker(index int, partition string, h *Handle, t *tracing.Tracer) *procPipelineWorker {
	...
	rruntime.Go(func() {
		...
		for subJob := range w.channel.store {
			if firstSubJob && !subJob.hasMore {
				subJob.statusDB = w.handle.procDB
