...
func New(warehouse model.Warehouse, db *sqlmiddleware.DB, uploader warehouseutils.Uploader, uploadID int64, warehouseManager WarehouseManager, loadFileDownloader downloader.Downloader, manager *encoding.Manager) *Identity {
	return &Identity{
		warehouse:        warehouse,
		db:               db,
		uploader:         uploader,
...
func (idr *Identity) applyRule(txn *sqlmiddleware.Tx, ruleID int64, gzWriter *misc.GZipWriter) (totalRowsModified int, err error) {
	...
	for _, row := range rows {
		eventLoader := idr.encodingManager.NewEventLoader(gzWriter, idr.uploader.GetLoadFileType(), idr.warehouse.Type)
...
func (idr *Identity) addRules(txn *sqlmiddleware.Tx, loadFileNames []string, gzWriter *misc.GZipWriter) (ids []int64, err error) {
	...
	for _, loadFileName := range loadFileNames {
		...
		eventReader := idr.encodingManager.NewEventReader(gzipReader, idr.warehouse.Type)
