...
func (bq *BigQuery) partitionColumn(workspaceID string) string {
	configKey := "Warehouse.bigquery.partitionColumn." + workspaceID
	return bq.conf.GetString(configKey, "")
}
...
func (bq *BigQuery) createTableView(ctx context.Context, tableName string, columnMap model.TableSchema) error {
	...
	if column := bq.partitionColumn(bq.warehouse.WorkspaceID); column != "" {
...
func (bq *BigQuery) LoadUserTables(ctx context.Context) (errorMap map[string]error) {
	...
	log.Infow("Starting load for identifies and users tables")
...
func (bq *BigQuery) DownloadIdentityRules(ctx context.Context, gzWriter *misc.GZipWriter) (err error) {
	getFromTable := func(tableName string) (err error) {
		...
		if hasAnonymousID && hasUserID {
			...
			bq.logger.Infon("anonymous_id, user_id columns not present in table: %s", logger.NewStringField(logfield.TableName, tableName))
			return nil
		}
		...
		for {
			...
			bq.logger.Infon("Downloading distinct combinations of anonymous_id, user_id", logger.NewIntField(logfield.TotalRows, totalRows))
			query := bq.db.Query(sqlStatement)
			job, err := bq.db.Run(ctx, query)
			if err != nil {
				break
