...
func TestIntegration(t *testing.T) {
	...
	var wifCredentials workloadIdentityTestCredentials
	require.NoError(t, jsonrs.Unmarshal([]byte(os.Getenv(workloadIdentityTestKey)), &wifCredentials))
	...
	t.Run("Workload identity federation", func(t *testing.T) {
		...
		t.Setenv("RUDDER_GCP_FEDERATION_AWS_ROLE_ARN", wifCredentials.AWSRoleARN)
		...
		newWarehouse := func(workspaceID, poolID, providerID, targetServiceAccount, namespace string) model.Warehouse {
			return model.Warehouse{
				...
				Destination: backendconfig.DestinationT{
					...
					Config: map[string]any{
						"project":                              wifCredentials.ProjectID,
						"location":                             wifCredentials.Location,
						"bucketName":                           wifCredentials.BucketName,
						"namespace":                            namespace,
						"authMethod":                           "workloadIdentityFederation",
						"workloadIdentityProjectNumber":        wifCredentials.ProjectNumber,
		...
		t.Run("service account impersonation", func(t *testing.T) {
			pool := wifCredentials.ServiceAccount

			t.Run("loads data", func(t *testing.T) {
				loadThroughFederation(t, newWarehouse(wifCredentials.WorkspaceID, pool.PoolID, pool.ProviderID, pool.TargetServiceAccount, whth.RandSchema(destType)))
			})
		...
		t.Run("federated identities", func(t *testing.T) {
			pool := wifCredentials.FederatedIdentities

			t.Run("loads data", func(t *testing.T) {
				loadThroughFederation(t, newWarehouse(wifCredentials.WorkspaceID, pool.PoolID, pool.ProviderID, "", whth.RandSchema(destType)))
			})
			t.Run("another workspace is denied", func(t *testing.T) {
