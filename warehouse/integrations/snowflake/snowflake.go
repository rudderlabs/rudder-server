...
func (sf *Snowflake) createTable(ctx context.Context, tableName string, columns model.TableSchema) (err error) {
	...
	sf.logger.Infow("Creating table in snowflake",
		obskit.DestinationID(sf.Warehouse.Destination.ID),
...
func (sf *Snowflake) createSchema(ctx context.Context) (err error) {
	...
	sf.logger.Infow(
		...
		obskit.Namespace(sf.Warehouse.Namespace),
		obskit.DestinationID(sf.Warehouse.Destination.ID),
		logger.NewStringField(lf.Query, sqlStatement),
	)
	_, err = sf.DB.ExecContext(ctx, sqlStatement)
...
func (sf *Snowflake) DeleteBy(ctx context.Context, tableNames []string, params whutils.DeleteByParams) error {
	...
	for _, tb := range tableNames {
		...
		if err != nil {
			log.Errorw("Cannot delete rows in snowflake table", obskit.Error(err))
			return err
		}
	}
	return nil
...
func (sf *Snowflake) loadTable(
	...
	log := sf.logger.With(
		obskit.SourceID(sf.Warehouse.Source.ID),
		obskit.SourceType(sf.Warehouse.Source.SourceDefinition.Name),
		obskit.DestinationID(sf.Warehouse.Destination.ID),
		obskit.DestinationType(sf.Warehouse.Destination.DestinationDefinition.Name),
		obskit.WorkspaceID(sf.Warehouse.WorkspaceID),
		obskit.Namespace(sf.Namespace),
	...
	if err != nil {
		log.Warnw("failed to sample duplicate rows", obskit.Error(err))
	}
	if len(duplicates) > 0 {
...
func (sf *Snowflake) LoadIdentityMergeRulesTable(ctx context.Context) error {
	log := sf.logger.With(logger.NewStringField(lf.TableName, identityMergeRulesTable))
	...
	if err != nil {
		log.Errorw("Error establishing connection for copying table", obskit.Error(err))
	...
	if regexErr == nil {
		log.Infow("Copying identity merge rules for table", logger.NewStringField(lf.Query, sanitisedSQLStmt))
	}

	if _, err = db.ExecContext(ctx, sqlStatement); err != nil {
		log.Errorw("Error while copying identity merge rules for table", obskit.Error(err))
...
func (sf *Snowflake) LoadIdentityMappingsTable(ctx context.Context) error {
	log := sf.logger.With(logger.NewStringField(lf.TableName, identityMappingsTable))
	log.Infow("Fetching load file location")
	...
	log = log.With(logger.NewStringField(lf.StagingTableName, stagingTableName))
	log.Infow("Creating staging table", logger.NewStringField(lf.Query, sqlStatement))
	...
	log.Infow("Adding autoincrement column", logger.NewStringField(lf.Query, sqlStatement))
	_, err = db.ExecContext(ctx, sqlStatement)
	...
	log.Infow("Copying identity mappings for table", logger.NewStringField(lf.Query, sqlStatement))
	_, err = db.ExecContext(ctx, sqlStatement)
	if err != nil {
	...
	log.Infow("Merge records for dedup for table", logger.NewStringField(lf.Query, sqlStatement))
	_, err = db.ExecContext(ctx, sqlStatement)
	if err != nil {
...
func (sf *Snowflake) LoadUserTables(ctx context.Context) map[string]error {
	...
	if _, err = resp.db.ExecContext(ctx, sqlStatement); err != nil {
		log.Warnw("failure creating staging table for users", obskit.Error(err))
		return map[string]error{
			identifiesTable: nil,
	...
	log.Infow("deduplication", logger.NewStringField(lf.Query, sqlStatement))
...
func (sf *Snowflake) CreateSchema(ctx context.Context) (err error) {
	...
	if err != nil {
		sf.logger.Errorw("Error checking if schema exists",
			obskit.Namespace(schemaIdentifier),
			obskit.Error(err),
		)
		return err
	}
	if schemaExists {
		sf.logger.Infow("Skipping schema creation (it already exists)",
			obskit.Namespace(schemaIdentifier),
		)
		return nil
	}
...
func (sf *Snowflake) AddColumns(ctx context.Context, tableName string, columnsInfo []whutils.ColumnInfo) (err error) {
	...
	log.Infow("Adding columns", logger.NewStringField(lf.Query, query))
		...
		if err != nil {
			if checkAndIgnoreAlreadyExistError(err) {
				log.Infow("Column already exists",
					logger.NewStringField(lf.ColumnName, columnsInfo[0].Name),
					obskit.Error(err),
				)
				err = nil
			}
...
func (sf *Snowflake) DownloadIdentityRules(ctx context.Context, gzWriter *misc.GZipWriter) (err error) {
	getFromTable := func(tableName string) (err error) {
		...
		if hasAnonymousID && hasUserID {
			...
			sf.logger.Infow("ANONYMOUS_ID, USER_ID columns not present in table", logger.NewStringField(lf.TableName, tableName))
			return nil
		}
