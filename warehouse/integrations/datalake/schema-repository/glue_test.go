...
func TestGlueSchemaRepositoryRoundTrip_V2(t *testing.T) {
	type S3Credentials struct {
		AccessKeyID string
		AccessKey   string
		Region      string
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			g, err := NewGlueSchemaRepositoryV2(config.New(), logger.NOP, warehouse)
			require.NoError(t, err)
			...
			t.Cleanup(func() {
				t.Log("Cleaning up")
				_, err = g.GlueClient.DeleteDatabase(ctx, &gluev2.DeleteDatabaseInput{
					Name: &testNamespace,
				})
				require.NoError(t, err)
			})
