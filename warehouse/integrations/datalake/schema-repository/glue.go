		...
		for _, table := range getTablesOutput.TableList {
			if table.Name != nil && table.StorageDescriptor != nil && table.StorageDescriptor.Columns != nil {
				...
				if _, ok := schema[tableName]; !ok {
					schema[tableName] = warehouseutils.TableSchema{}
				}
...
func (gl *GlueSchemaRepository) getStorageDescriptor(tableName string, columnMap warehouseutils.TableSchema) *glue.StorageDescriptor {
