...
func TestManager_InsertJobHandler(t *testing.T) {
	...
	tableUploadsRepo := repo.NewTableUploads(db, config.New(), stats.NOP, repo.WithNow(func() time.Time {
		return now
	}))
...
func TestManager_StatusJobHandler(t *testing.T) {
	...
	sourceRepo := repo.NewSource(db, stats.NOP, repo.WithNow(func() time.Time {
		return now
	}))
