...
func newWorker(
	...
	s.uploadJobFactory = uploadjob.NewFactory(reporting, db, validations.NewDestinationValidator(), lfGenerator, s.conf, s.log, s.statsFactory, s.encodingFactory)
...
func (w *worker) start(ctx context.Context, notificationChan <-chan *notifier.ClaimJob, slaveID string) {
	...
	for {
		...
		case claimedJob, ok := <-notificationChan:
			...
			case notifier.JobTypeUploadSM:
				w.processClaimedUploadStateMachineJob(ctx, claimedJob)
			default:
...
func (w *worker) processClaimedUploadStateMachineJob(ctx context.Context, claimedJob *notifier.ClaimJob) {
	...
	uploadJob := w.uploadJobFactory.NewUploadJob(ctx, &model.UploadJob{
		Upload:       job.Upload,
