...
func (t *transformer) identifyCommonProps(pi *processingInfo) (map[string]any, map[string]string, error) {
	...
	if err := t.setDataAndColumnTypeFromInput(pi, pi.event.Message["userProperties"], commonProps, commonColumnTypes,
		"identify_userProperties_", 2, "", 0,
	); err != nil {
	...
	if pi.dstOpts.allowUsersContextTraits {
		...
		if err := t.setDataAndColumnTypeFromInput(pi, contextTraits, commonProps, commonColumnTypes,
			"identify_context_traits_", 3, "", 0,
		); err != nil {
			return nil, nil, fmt.Errorf("identify common props:setting data and column types from message: %w", err)
		}
	}
	if err := t.setDataAndColumnTypeFromInput(pi, pi.event.Message["traits"], commonProps, commonColumnTypes,
		"identify_traits_", 2, "", 0,
	); err != nil {
		return nil, nil, fmt.Errorf("identify common props:setting data and column types from message: %w", err)
	}
	if err := t.setDataAndColumnTypeFromInput(pi, pi.event.Message["context"], commonProps, commonColumnTypes,
		"identify_context_", 2, "context_", 0,
	); err != nil {
		return nil, nil, fmt.Errorf("identify common props:setting data and column types from message: %w", err)
	}
