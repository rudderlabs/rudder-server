	...
	for _, destType := range whutils.WarehouseDestinations {
		...
		for _, destType := range whutils.WarehouseDestinations {
			for _, opt := range integrationOpts {
				...
				f.Add(destType, `{}`, fmt.Sprintf(`{"type":"EXTRACT","recordId":"recordID","event":"users","receivedAt":"2021-09-01T00:00:00.000Z","properties":{"name":"Home","title":"Home | RudderStack","url":"https://www.rudderstack.com"},"context":{"traits":{"name":"Richard Hendricks","email":"rhedricks@example.com","logins":2},"ip":"1.2.3.4"},%s}`, itrOptsPayload))
			}
		...
		f.Fuzz(func(t *testing.T, destType, destConfigJSON, payload string) {
			...
			destinationTransformer := ptrans.NewTransformer(conf, logger.NOP, stats.Default)
