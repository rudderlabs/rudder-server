...
func TestSlaveJob(t *testing.T) {
	...
	t.Run("download staging file", func(t *testing.T) {
		...
		t.Run("download", func(t *testing.T) {
			...
			jr := newJobRun(p, config.Default, logger.NOP, stats.Default, encoding.NewManager(config.Default))
		...
		t.Run("context cancelled", func(t *testing.T) {
			...
			jr := newJobRun(p, config.Default, logger.NOP, statsStore, encoding.NewManager(config.Default))
	...
	t.Run("upload load files", func(t *testing.T) {
		...
		for _, tc := range testCases {
			...
			t.Run(tc.name, func(t *testing.T) {
				...
				jr := newJobRun(job, c, logger.NOP, store, encoding.NewManager(config.Default))
