...
func TestCleanupObjectStorageFiles(t *testing.T) {
	...
	t.Run("GCS cleanup enabled, chunked deletion", func(t *testing.T) {
		...
		job.config.maxParallelObjDeleteRequests = func(workspaceId, objStorageType string) int {
			return 2 // Allow 2 parallel delete requests
		}
		job.config.objDeleteBatchSize = func(workspaceId, objStorageType string) int {
			return 4 // Use 4 as batch size
		}
