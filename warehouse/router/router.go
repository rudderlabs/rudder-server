	...
	for i := 0; i < r.config.maxConcurrentUploadJobs; i++ {
		r.backgroundGroup.Go(func() error {
			for uploadJob := range workerChan {
				...
				err := uploadJob.run()
				if err != nil {
					r.logger.Errorn("[WH] Failed in handle Upload jobs for worker", obskit.Error(err))
				}
...
loop:
	for {
		...
		case <-r.bcManager.InitialConfigFetched:
			r.logger.Debugn("Initial config fetched in runUploadJobAllocator", logger.NewStringField(logfield.DestinationType, r.destType))
		}
...
func (r *Router) uploadsToProcess(ctx context.Context, availableWorkers int, skipIdentifiers []string) ([]*UploadJob, error) {
	...
	for _, upload := range uploads {
		...
		if upload.WorkspaceID == "" {
			...
			if !ok {
				r.logger.Warnn("could not find workspace id for source id", logger.NewStringField(logfield.SourceID, upload.SourceID))
			}
		}
		...
		if !found {
			...
			r.logger.Errorn("Error in upload jobs processing", obskit.Error(err))
			continue
		}
	...
	for {
		...
		for _, warehouse := range r.warehouses {
			...
			rruntime.GoForWarehouse(func() {
				...
				r.logger.Debugn("[WH] Processing Jobs for warehouse", logger.NewStringField("warehouseIdentifier", w.Identifier))
...
func (r *Router) createJobs(ctx context.Context, warehouse model.Warehouse) (err error) {
	...
	if len(stagingFilesList) == 0 {
		r.logger.Debugn("[WH]: Found no pending staging files", logger.NewStringField("warehouseIdentifier", warehouse.Identifier))
		return nil
	}
