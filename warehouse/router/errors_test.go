...
func TestErrorHandler_MatchUploadJobErrorType(t *testing.T) {
	t.Run("known errors", func(t *testing.T) {
		testCases := []struct {
			...
			{
				"BigQuery too many total leaf fields", warehouseutils.BQ, errors.New("{\"exporting_data_failed\":{\"attempt\":5,\"errors\":[\"1 errors occurred: update schema: adding columns to warehouse: failed to add columns for table device_data_event in namespace event of destination BQ: *** with response: googleapi: Error 400: Too many total leaf fields: ***, max allowed field count: 10000 , invalid\"]}}"), model.ColumnCountError,
			},
			...
			{
				"ClickHouse exceeded memory limits", warehouseutils.CLICKHOUSE, errors.New("{\"exporting_data_failed\":{\"attempt\":1,\"errors\":[\"Error occurred while committing with response: Error while committing transaction as there was response while loading in table with response:code: 241, message: Memory limit (total) exceeded: would use 27.01 GiB (attempt to allocate chunk of 4606752 bytes), maximum: 27.00 GiB: (avg_value_size_hint = 45, avg_chars_size = 44.4, limit = 8192)\"]}}"), model.InsufficientResourceError,
			},
			...
			{
				"Deltalake user does not have permission to read files", warehouseutils.DELTALAKE, errors.New("{\"exporting_data_failed\":{\"attempt\":5,\"errors\":[\"response while executing with response: [42000] [Simba][Hardy] (80) Syntax or semantic analysis response thrown in server while executing query. Error message from server: org.apache.hive.service.cli.HiveSQLException: Error running query: com.databricks.sql.managedcatalog.acl.UnauthorizedAccessException: PERMISSION_DENIED: User does not have READ FILES on External Location ***.\tat org.apache.spark.sql.hive.t (80) (SQLExecDirectW)\"]}}"), model.PermissionError,
			},
			{
				"Deltalake user does not have permission to create on catalog", warehouseutils.DELTALAKE, errors.New("{\"creating_remote_schema_failed\":{\"attempt\":2,\"errors\":[\"response while executing with response: [42000] [Simba][Hardy] (80) Syntax or semantic analysis response thrown in server while executing query. Error message from server: org.apache.hive.service.cli.HiveSQLException: Error running query: java.lang.SecurityException: User does not have permission CREATE on CATALOG.\tat org.apache.spark.sql.hive.thriftserver.HiveThriftServerErrors$.runningQueryError(HiveThriftServerErrors.sc (80) (SQLExecDirectW)\"]}}"), model.PermissionError,
			},
			{
				"Deltalake endpoint not found", warehouseutils.DELTALAKE, errors.New("{\"fetching_remote_schema_failed\":{\"attempt\":5,\"errors\":[\"fetching schema from warehouse: fetching schema: creating schema: checking if schema exists: schema exists: databricks: request response: response connecting: host=*** port=***, httpPath=***: databricks: request response: open session request response: Post ***: ENDPOINT_NOT_FOUND: SQL warehouse *** does not exist at all in the database: request response after 1 attempt(s): unexpected HTTP status 404 Not Found\"]}}"), model.ResourceNotFoundError,
			},
			{
				"Deltalake resource does not exist", warehouseutils.DELTALAKE, errors.New("{\"fetching_remote_schema_failed\":{\"attempt\":2,\"errors\":[\"fetching schema from warehouse: fetching schema: creating schema: checking if schema exists: schema exists: databricks: request response: response connecting: host=*** port=443, httpPath=***: databricks: request response: open session request response: Post ***: RESOURCE_DOES_NOT_EXIST: No cluster found matching: ***: request response after 1 attempt(s): unexpected HTTP status 404 Not Found\"]}}"), model.ResourceNotFoundError,
			},
			...
			{
				"Postgres tables can have at most 1600 columns", warehouseutils.POSTGRES, errors.New("{\"exporting_data_failed\":{\"attempt\":1,\"errors\":[\"failed to add columns for table *** in namespace *** of destination POSTGRES:*** with response: pq: tables can have at most 1600 columns\"]}}"), model.ColumnCountError,
			},
			{
			...
			{
				"RedShift eof", warehouseutils.RS, errors.New("{\"internal_processing_failed\":{\"attempt\":5,\"errors\":[\"redshift set query_group response : EOF\"]}}"), model.PermissionError,
			},
			{
			...
			{
				"RedShift tables can have at most 1600 columns", warehouseutils.RS, errors.New("{\"exporting_data_failed\":{\"attempt\":11,\"errors\":[\"update schema: adding columns to warehouse: failed to add columns for table *** in namespace *** of destination *** with response: pq: tables can have at most 1600 columns\"]}}"), model.ColumnCountError,
			},

			{
				"Snowflake requested warehouse does not exist or not authorized", warehouseutils.SNOWFLAKE, errors.New("{\"internal_processing_failed\":{\"attempt\":1,\"errors\":[\"SF: snowflake alter session response : (390201 (08004): The requested warehouse does not exist or not authorized.)\"]}}"), model.PermissionError,
			},
			{
				"Snowflake requested database does not exist or not authorized", warehouseutils.SNOWFLAKE, errors.New("{\"internal_processing_failed\":{\"attempt\":9,\"errors\":[\"SF: snowflake alter session response : (390420 (08004): IP *** is not allowed to access Snowflake. Contact your local security administrator or please create a case with Snowflake Support or reach us on our support line\"]}}"), model.PermissionError,
			},
			{
				"Snowflake verify account name is correct", warehouseutils.SNOWFLAKE, errors.New("{\"internal_processing_failed\":{\"attempt\":5,\"errors\":[\"SF: snowflake alter session response : (390201 (08004): The requested database does not exist or not authorized.)\"]}}"), model.PermissionError,
			},
			{
				"Snowflake incorrect username or password was specified", warehouseutils.SNOWFLAKE, errors.New("{\"internal_processing_failed\":{\"attempt\":3,\"errors\":[\"SF: snowflake alter session response : (260008 (08004): failed to connect to db. verify account name is correct.)\"]}}"), model.PermissionError,
			},
			{
				"Snowflake insufficient privileges to operate on table", warehouseutils.SNOWFLAKE, errors.New("{\"exporting_data_failed\":{\"attempt\":5,\"errors\":[\"failed to add columns for table *** in namespace *** of destination *** with response: 003001 (42501): SQL access control response:\nInsufficient privileges to operate on table\"]}}"), model.PermissionError,
			},
			{
				"Snowflake ip is not allowed to access Snowflake", warehouseutils.SNOWFLAKE, errors.New("{\"internal_processing_failed\":{\"attempt\":6,\"errors\":[\"SF: snowflake alter session response : (390100 (08004): Incorrect username or password was specified.)\"]}}"), model.PermissionError,
			},
			{
				"Snowflake user temporarily locked", warehouseutils.SNOWFLAKE, errors.New("{\"internal_processing_failed\":{\"attempt\":4,\"errors\":[\"SF: snowflake alter session response : (390102 (08004): User temporarily locked. Contact your local system administrator or please create a case with Snowflake Support or reach us on our support line\"]}}"), model.PermissionError,
			},
			{
				"Snowflake schema already exists, but current role has no privileges on it", warehouseutils.SNOWFLAKE, errors.New("{\"creating_remote_schema_failed\":{\"attempt\":5,\"errors\":[\"003041 (42710): SQL compilation response:Schema *** already exists, but current role has no privileges on it. If this is unexpected and you cannot resolve this problem, contact your system administrator. ACCOUNTADMIN role may be required to manage the privileges on the object.\"]}}"), model.PermissionError,
			},
			{
			...
			{
				"Snowflake location is not allowed by integration", warehouseutils.SNOWFLAKE, errors.New("{\"exporting_data_failed\":{\"attempt\":4,\"errors\":[\"003127 (42601): SQL compilation response:Location *** is not allowed by integration ***. Please use DESC INTEGRATION to check out allowed and blocked locations.\"]}}"), model.PermissionError,
			},
			{
	...
	t.Run("UnKnown errors", func(t *testing.T) {
		...
		er := &router.ErrorHandler{Mapper: m}
		jobErrorType := er.MatchUploadJobErrorType(errors.New("unknown response"))
