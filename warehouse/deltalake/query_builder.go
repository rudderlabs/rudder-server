...
func mergeableLTSQLStatement(namespace, tableName, stagingTableName string, columnKeys []string, partitionedQuery string) (sqlStatement string) {
	pk := primaryKey(tableName)
	if partitionedQuery != "" {
		partitionedQuery += " AND"
	}
