...
func (wh *HandleT) populateHistoricIdentities(warehouse warehouseutils.Warehouse) {
	...
	rruntime.GoForWarehouse(func() {
		...
		if err != nil {
			_, _ = job.setUploadError(err, Aborted)
			return
		}
		...
		_ = job.setUploadStatus(UploadStatusOpts{Status: getInProgressState(ExportedData)})
		loadErrors, err := job.loadIdentityTables(true)
		if err != nil {
			pkgLogger.Errorf(`[WH]: Identity table upload errors: %v`, err)
		}
		if len(loadErrors) > 0 {
			_, _ = job.setUploadError(misc.ConcatErrors(loadErrors), Aborted)
			return
		}
		_ = job.setUploadStatus(UploadStatusOpts{Status: ExportedData})
	})
}
