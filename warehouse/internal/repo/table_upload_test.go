...
func TestTableUploadRepo(t *testing.T) {
	...
	t.Run("TotalEvents", func(t *testing.T) {
		...
		//t.Run("populate total events", func(t *testing.T) {
		//	err := r.PopulateTotalEventsFromStagingFileIDs(ctx, uploadID, tables[0])
		//	require.NoError(t, err)
		//
		//	tableUpload, err := r.GetByUploadIDAndTableName(ctx, uploadID, tables[0])
		//	require.NoError(t, err)
		//	require.Equal(t, int64(1), tableUpload.TotalEvents)
		//})
	})
}
