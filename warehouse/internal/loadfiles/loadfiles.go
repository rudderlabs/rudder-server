...
func (lf *LoadFileGenerator) createUploadJobs(ctx context.Context, job *model.UploadJob, stagingFiles []*model.StagingFile, publishBatchSize int, uniqueLoadGenID string) error {
	...
	for _, chunk := range lo.Chunk(stagingFiles, publishBatchSize) {
		ch, err := lf.publishJobs(ctx, job, uniqueLoadGenID, chunk, destinationRevisionIDMap, false, notifier.JobTypeUpload, func(baseReq baseWorkerJobRequest, stagingFile *model.StagingFile) (stdjson.RawMessage, error) {
...
func (lf *LoadFileGenerator) createUploadV2Jobs(ctx context.Context, job *model.UploadJob, stagingFiles []*model.StagingFile, publishBatchSize int, uniqueLoadGenID string) error {
	...
	for _, chunk := range lo.Chunk(stagingFiles, publishBatchSize) {
		...
		ch, err := lf.publishJobs(ctx, job, uniqueLoadGenID, chunk, destinationRevisionIDMap, true, notifier.JobTypeUploadV2, func(baseReq baseWorkerJobRequest, stagingFile *model.StagingFile) (stdjson.RawMessage, error) {
		...
		g.Go(func() error {
			return lf.processNotifierResponseV2(ctx, ch, job, chunk)
		})
	}
	return g.Wait()
}
