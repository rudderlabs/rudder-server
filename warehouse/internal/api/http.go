...
type stagingFileSchemaSnapshotGetter interface {
	Get(ctx context.Context, sourceID, destinationID, workspaceID string, schemaBytes json.RawMessage) (*model.StagingFileSchemaSnapshot, error)
}
...
func (api *WarehouseAPI) processHandler(w http.ResponseWriter, r *http.Request) {
	...
	if api.EnableStagingFileSchemaSnapshot.Load() {
		snapshot, err := api.StagingFileSchemaSnapshotGetter.Get(
			r.Context(),
