...
func loadCacheConfig() {
	config.RegisterIntConfigVariable(2, &CacheMaxSize, true, 1, "TransformationDebugger.maxTransformationsCacheSize")
	config.RegisterDurationConfigVariable(720 * time.Hour, &CacheTTL, true, 1, "TransformationDebugger.maxTransformationsCacheTTL")
}
...
func (c *Cache) update(key string, value *TransformationStatusT) {
	c.init()
	c.lock.Lock()
	defer c.lock.Unlock()

	if _, ok := c.m[key]; !ok {
		c.m[key] = &cacheItem{objs: make([]*TransformationStatusT, 0, c.Size)}
	}
...
func (c *Cache) readAndPopData(key string) []*TransformationStatusT {
	c.init()
...
func (c *Cache) readData(key string) []*TransformationStatusT {
	c.init()
	var historicEventsDelivery []*TransformationStatusT
	c.lock.Lock()
	if deliveryStatus, ok := c.m[key]; ok {
