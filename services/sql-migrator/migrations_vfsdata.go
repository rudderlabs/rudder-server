...
var MigrationAssets = func() http.FileSystem {
	fs := vfsgen۰FS{
		...
		"/warehouse": &vfsgen۰DirInfo{
			name:    "warehouse",
			modTime: time.Date(2021, 8, 5, 9, 41, 13, 222951558, time.UTC),
		},
		...
		"/warehouse/000011_add_timeWindow_to_wh_staging_files.up.sql": &vfsgen۰FileInfo{
			name:    "000011_add_timeWindow_to_wh_staging_files.up.sql",
			modTime: time.Date(2021, 8, 5, 9, 42, 55, 724760608, time.UTC),
	...
	fs["/warehouse"].(*vfsgen۰DirInfo).entries = []os.FileInfo{
		...
		fs["/warehouse/000011_add_timeWindow_to_wh_staging_files.up.sql"].(os.FileInfo),
	}
