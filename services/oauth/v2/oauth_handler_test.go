...
var _ = Describe("Oauth", func() {
	...
	Describe("Test AuthStatusToggle function", func() {
		It("authStatusToggle function call when config backend api call failed", func() {
			...
			err := oauthHandler.AuthStatusToggle(&v2.StatusRequestParams{
			...
			Expect(err.StatusCode()).To(Equal(http.StatusBadRequest))
			Expect(err.Error()).To(Equal("status 400: message: unable to update the auth status for the destination"))
		})
		...
		It("authStatusToggle function call when config backend api call failed due to config backend service is failed due to timeout", func() {
			...
			Expect(err).To(Not(BeNil()))
			Expect(err.StatusCode()).To(Equal(http.StatusInternalServerError))
			Expect(err.Error()).To(Equal("status 500: message: mock mock 127.0.0.1:1234->127.0.0.1:12340: read: " + timeoutError))
		})
	})
