...
func (producer *EventBridgeProducerV2) Produce(jsonData json.RawMessage, _ interface{}) (int, string, string) {
	...
	if err != nil {
		...
		pkgLogger.Errorn("[EventBridge] error", logger.NewIntField("statusCode", int64(statusCode)), logger.NewStringField("respStatus", respStatus), logger.NewStringField("responseMessage", responseMessage))
