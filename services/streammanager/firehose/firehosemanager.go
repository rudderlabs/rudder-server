...
func NewProducer(destinationConfig interface{}, o common.Opts) (*FireHoseProducer, error) {
	...
	return &FireHoseProducer{client: firehose.New(awsutils.CreateSession(sessionConfig))}, nil
}

// Produce creates a producer and send data to Firehose.
func (producer *FireHoseProducer) Produce(jsonData json.RawMessage, destConfig interface{}) (int, string, string) {
	...
	if errorRec != nil {
		...
		pkgLogger.Errorf("[FireHose] error  :: %s : %s", respStatus, responseMessage)
