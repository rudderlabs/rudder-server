...
func (c *otelCounter) Count(n int) {
	c.counter.Add(context.TODO(), int64(n), c.tags.otelAttributes()...)
}
...
func (c *otelCounter) Increment() {
	c.counter.Add(context.TODO(), 1, c.tags.otelAttributes()...)
}
...
func (t *otelTimer) SendTiming(duration time.Duration) {
	t.timer.Record(context.TODO(), duration.Milliseconds(), t.tags.otelAttributes()...)
}
...
func (h *otelHistogram) Observe(value float64) {
	h.histogram.Record(context.TODO(), value, h.tags.otelAttributes()...)
}
