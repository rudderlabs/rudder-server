...
func (n *Notifier) setupTables() error {
	...
	err := backoff.RetryNotify(operation, backoffWithMaxRetry, func(err error, t time.Duration) {
		n.logger.Warnn("retrying warehouse database migration", logger.NewDurationField("retryAfter", t), obskit.Error(err))
	})
	if err != nil {
...
func (n *Notifier) Publish(
	...
	n.logger.Infon("Inserted records into queue for batch",
		logger.NewIntField("recordCount", int64(len(publishRequest.Payloads))),
		logger.NewStringField("queueName", queueName),
		logger.NewStringField("batchID", batchID))
...
func (n *Notifier) trackBatch(
	...
	n.background.group.Go(func() error {
		...
		for {
			...
			n.logger.Infon("Completed processing all files in batch", logger.NewStringField("batchID", batchID))
...
func (n *Notifier) RunMaintenance(ctx context.Context) error {
	...
	for {
		...
		if len(orphanJobIDs) > 0 {
			n.logger.Infon("Re-triggered job ids", logger.NewIntSliceField("orphanJobIDs", orphanJobIDs))
		}
