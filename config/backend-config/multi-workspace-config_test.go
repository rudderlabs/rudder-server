...
var _ = Describe("workspace-config", func() {
	...
	Context("Get method", func() {
		...
		It("Expect to execute request with the correct body and headers and return successfull response", func() {
			...
			mockHttp.EXPECT().NewRequest("GET", fmt.Sprintf("%s/cachedHostedWorkspaceConfig?fetchAll=true", configBackendURL), nil).Return(testRequest, nil).Times(1)
		...
		It("Expect to make the correct actions if fail to create the request", func() {
			mockHttp.EXPECT().NewRequest("GET", fmt.Sprintf("%s/cachedHostedWorkspaceConfig?fetchAll=true", configBackendURL), nil).Return(nil, errors.New("TestError")).AnyTimes()
		...
		It("Expect to make the correct actions if fail to send the request", func() {
			...
			mockHttp.EXPECT().NewRequest("GET", fmt.Sprintf("%s/cachedHostedWorkspaceConfig?fetchAll=true", configBackendURL), nil).Return(testRequest, nil).AnyTimes()
