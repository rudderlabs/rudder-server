...
func (mpe *migrationJobExecutor) Run(ctx context.Context) error {
	err := func() error {
		...
		// open the stream
		mpe.logger.Debugn("opening the stream")
		...
		// allow only one unacked batch to be added at a time
		// channel is closed when sender is done
		...
		batchesToAck := make(chan lo.Tuple2[int, []*jobsdb.JobT]) // channel for sending unacked batch info to the receiver goroutine to acknowledge (one at a time)
		...
		streamGroup.Go(func() error {
			...
			for {
				...
				// send the batch info to the receiver goroutine
				...
				for i, chunk := range chunks {
					...
					if lastChunk { // send batch info to receiver only on last chunk
						select {
	...
	if err != nil {
		mpe.logger.Errorn("Partition migration job execution failed", obskit.Error(err))
	}
	return err
}
