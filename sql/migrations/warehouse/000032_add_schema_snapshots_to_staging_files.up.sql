...
ADD COLUMN IF NOT EXISTS schema_snapshot_id UUID REFERENCES wh_staging_file_schema_snapshots(id) ON DELETE
SET NULL,
    -- Link to the schema snapshot
ADD COLUMN IF NOT EXISTS schema_snapshot_patch TEXT;
-- JSON Patch (RFC 6902) diff from the snapshot to the actual schema
...
CREATE INDEX IF NOT EXISTS wh_staging_file_schema_snapshots_source_id_destination_id_created_at_index ON wh_staging_file_schema_snapshots(source_id, destination_id, created_at DESC);