...
//  2. Only the still-pending consumers' statuses are carried over; terminal consumers
//     are trimmed from the job's consumers array and their status rows are not copied.
//  3. The destination registry is seeded with exactly the (trimmed) consumers of moved jobs.
