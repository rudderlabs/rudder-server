...
func (jd *HandleT) getProcessedJobsDS(ds dataSetT, getAll bool, stateFilters []string,
	...
	if jd.isEmptyResult(ds, stateFilters, customValFilters, parameterFilters) {
		if misc.ContainsString(customValFilters, "RS") || misc.ContainsString(customValFilters, "S3") {
			logger.Infof("[getProcessedJobsDS] Empty cache hit for ds: %v, stateFilters: %v, customValFilters: %v, parameterFilters: %v", ds, stateFilters, customValFilters, parameterFilters)
		}
		return []*JobT{}, nil
	}
