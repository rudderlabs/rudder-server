...
func (jd *HandleT) createDS(newDS dataSetT, l lock.DSListLockToken) {
	...
	sqlStatement := fmt.Sprintf(`CREATE TABLE "%s" (
                                      job_id BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY (START WITH 1),
...
func (jd *HandleT) setSequenceNumber(l lock.DSListLockToken, newDSIdx string) dataSetT {
	...
	if len(dRangeList) > 0 {
		...
		sqlStatement := fmt.Sprintf(`ALTER TABLE "%[1]s" ALTER COLUMN job_id SET MINVALUE %[2]d SET START WITH %[2]d RESTART WITH %[2]d`,
			jd.tablePrefix+`_jobs_`+fmt.Sprint(newDSIdx), newDSMin+1)
		_, err := jd.dbHandle.Exec(sqlStatement)
		jd.assertError(err)
	}
