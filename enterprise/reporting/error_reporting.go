...
type edReportingClient interface {
	SendErrorMetric(ctx context.Context, metric *types.EDMetric) error
}
...
type ErrorDetailReporter struct {
	...
	reportingClient  edReportingClient
...
func NewErrorDetailReporter(
	...
	reportingClient := NewClient(reportingServiceURL, conf, log, stats)
		...
		for {
			...
			for _, metric := range metrics {
				...
				errGroup.Go(func() error {
					err := edr.reportingClient.SendErrorMetric(errCtx, metricToSend)
					if err != nil {
