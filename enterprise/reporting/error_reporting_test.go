...
func TestGetErrorMessage_NestedJSONResponse(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expected: "There were 9 invalid conversion events. None were processed.",
		},
		{
...
func TestGetErrorMessage_ComplexNestedStructures(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			expected: "There were 2 invalid conversion events. None were processed.",
		},
		{
...
func TestGetErrorMessage_RealSampleData(t *testing.T) {
	ext := NewErrorDetailExtractor(logger.NOP)

	// Test with the actual sample error JSON from INT-3995 (sanitized)
	sampleErrorJSON := `{
		"response": "{\"message\":\"There were 9 invalid conversion events. None were processed.\",\"invalid_events\":[{\"error_message\":\"event_at timestamp must be less than 168h0m0s old\",\"event\":{\"click_id\":\"\",\"event_at\":\"2025-07-10T17:48:14.266Z\",\"event_type\":{\"tracking_type\":\"PageVisit\"},\"event_metadata\":{\"products\":[{\"name\":\"\"}],\"conversion_id\":\"test-conversion-id\"},\"user\":{\"ip_address\":\"test-ip\",\"user_agent\":\"test-user-agent\",\"screen_dimensions\":{\"width\":430,\"height\":932},\"uuid\":\"test-uuid\"}}}]}",
		"dontBatch": true,
		"firstAttemptedAt": "2025-08-02T00:04:10.049Z",
		"content-type": "application/json",
		"routerSubStage": "router_dest_delivery",
	...
	expected := "There were 9 invalid conversion events. None were processed."
	result := ext.GetErrorMessage(sampleErrorJSON)
