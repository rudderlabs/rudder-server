...
func newWorker(
	...
	w.config.instanceID = conf.GetStringVar("1", "INSTANCE_ID")
	w.config.bucketName = conf.GetStringVar("rudder-failed-messages", "ErrorIndex.Storage.Bucket")
	...
	w.config.minWorkerSleep = conf.GetDurationVar(1, time.Minute, "Reporting.errorIndexReporting.minWorkerSleep")
	w.config.uploadFrequency = conf.GetDurationVar(5, time.Minute, "Reporting.errorIndexReporting.uploadFrequency")
