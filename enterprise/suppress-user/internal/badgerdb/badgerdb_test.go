...
func TestBadgerRepository(t *testing.T) {
	...
	t.Run("test if backup & restore is working properly", func(t *testing.T) {
		...
		con := func() bool {
			suppressed, err := repo.Suppressed("workspace1", "user1", "src-1")
			require.NoError(t, err)
			return suppressed
		}
		require.Eventually(t, con, 5*time.Second, 100*time.Millisecond, "abc")

		tmp := []byte{}
		err = repo.Backup(bytes.NewBuffer(tmp))
		require.NoError(t, err)
		repo2, err := badgerdb.NewRepository(path.Join(t.TempDir(), strings.ReplaceAll(uuid.New().String(), "-", "")), logger.NOP)
		require.NoError(t, err)
		err = repo2.Restore(bytes.NewBuffer(tmp))
		require.NoError(t, err)

		con2 := func() bool {
			suppressed, err := repo2.Suppressed("workspace1", "user1", "src-1")
			require.NoError(t, err)
			return suppressed
		}
		require.Eventually(t, con2, 5*time.Second, 100*time.Millisecond, "abc")
	})
