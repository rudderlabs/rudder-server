				...
				for {
					...
					case batchJobs := <-brt.processQ:
						...
						case misc.ContainsString(objectStorageDestinations, brt.destType):
							destUploadStat := stats.NewBatchRtDestWithParamStat(`batch_router_dest_upload_time`, stats.TimerType, "destType", brt.destType)
						...
						case misc.ContainsString(warehouseDestinations, brt.destType):
							destUploadStat := stats.NewBatchRtDestWithParamStat(fmt.Sprintf(`batch_router.%s_dest_upload_time`, warehouseutils.ObjectStorageMap[brt.destType]), stats.TimerType, "destType", brt.destType)
							destUploadStat.Start()
