...
func newManagerInternal(log logger.Logger, statsFactory stats.Stats, destination *backendconfig.DestinationT, oauthClientV2 oauthv2.Authorizer) (*BingAdsBulkUploader, error) {
	...
	clientNew := Client{}
	bingUploader := NewBingAdsBulkUploader(log, statsFactory, destination.DestinationDefinition.Name, bingads.NewBulkService(session), &clientNew, initializationError)
