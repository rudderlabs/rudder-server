...
func New(
	...
	m.config.client.maxHTTPConnections = conf.GetIntVar(10, 1, "SnowpipeStreaming.Client.maxHTTPConnections")
...
func (m *Manager) Upload(_ context.Context, asyncDest *common.AsyncDestinationStruct) common.AsyncUploadOutput {
	...
	if err != nil {
		m.logger.Warnn("Failed to prepare discards channel",
	...
	if len(importInfos) > 0 {
		...
		if err != nil {
			m.logger.Warnn("Failed to marshal import id",
				obskit.Error(err),
			)
...
func (m *Manager) GetUploadStats(input common.GetUploadStatsInput) common.GetUploadStatsResponse {
	...
	if err != nil {
		m.logger.Warnn("Failed to unmarshal failed job parameters",
			obskit.Error(err),
		)
