...
var _ = Describe("BatchRouter", func() {
	...
	Context("Initialization", func() {
		...
		It("should initialize and recover after crash", func() {
			...
			batchrouter.Setup(context.Background(), c.mockBackendConfig, c.mockBatchRouterJobsDB, c.mockProcErrorsDB, s3DestinationDefinition.Name, nil, c.mockMultitenantI, transientsource.NewEmptyService())
		})
	})
	...
	Context("normal operation - s3 - do not readPerDestination", func() {
		...
		It("should send failed, unprocessed jobs to s3 destination", func() {
			...
			c.mockBatchRouterJobsDB.EXPECT().JournalDeleteEntry(context.Background(), gomock.Any()).Times(1)
