...
func NewThrottler(destType, destinationID, eventType string, limiter types.Limiter, config *config.Config, stat stats.Stats) types.Throttler {
	return switcher.NewThrottlerSwitcher(
		...
		NewAllEventTypesThrottler(destType, destinationID, limiter, config, stat),
		NewPerEventTypeThrottler(destType, destinationID, eventType, limiter, config, stat),
	)
}
