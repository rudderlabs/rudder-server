...
func NewThrottler(destType, destinationID, eventType string,
	...
	return switcher.NewThrottlerSwitcher(
		...
		NewAllEventTypesThrottler(destType, destinationID, allEventsAlgorithm, limiter, config, stat),
		NewPerEventTypeThrottler(destType, destinationID, eventType, perEventAlgorithm, limiter, config, stat),
	)
}
